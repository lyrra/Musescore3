...
; o->m
(define (emit-slotref-ptr ir args)
  (let ((o (car ir))
        (m (cadr ir)))
    (emit-c o)
    (format %c "->")
...
(define (emit-c-ref ir args)
  (format %c "&")
  (emit-c (car ir))
  '())

(define (emit-c-deref ir args)
  (format %c "*")
  (emit-c (car ir))
  '())

(define (emit-c-cast ir args)
  (if (memq 'tcp args)
      (format %c "  return "))
  (let ((type (car ir))
        (e (cadr ir)))
  ...
  (cond
   ((pair? ir)
    (case (car ir)
      ...
      ((&&) (emit-infix '&& (cdr ir) args))
      ((>=) (emit-infix '>= (cdr ir) args))
      ((->) (emit-slotref-ptr (cdr ir) args))
      ((.>) (emit-slotref (cdr ir) args))
      ((ref)
       (emit-c-ref (cdr ir) args))
      ((deref)
       (emit-c-deref (cdr ir) args))
      ((infix)
       (emit-infix (cadr ir) (cddr ir) args))
      ((cast)
       (emit-c-cast (cdr ir) args))
      ((c++new)
       (emit-c-c++new (cdr ir) args))
      ((raw)
